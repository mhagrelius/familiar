...
from gjs.guide, vendored verbatim so it applies to anything editing the two
GNOME Shell extensions under this directory. Refresh it from [the upstream
Markdown][best-practices-markdown] when upstream changes.

## This project

Two extensions live here and neither is part of the Familiar application. The
Rust app is governed by the repo's CLAUDE.md; these rules apply to the JavaScript
below this directory only.

**`shell-extension/` — Familiar's schedules in Quick Settings.** It reads the
`us.hagreli.Familiar.Jobs` D-Bus interface and pushes rather than polls;
`daemon/jobs.js` holds the proxy, `ui/toggle.js` the widgets, and `extension.js`
nothing but enable/disable. Familiar not running is an ordinary state, not an
error.

**`webcam-extension/` — an Insta360 Link webcam's noise cancelling.** It talks to
a webcam, not the assistant, and lives here only because that is where the
problem was found.

**`camera.js` spawns `link-ctl` on purpose.** Insta360 ships no Linux control app
and ALSA exposes nothing but capture volume for this device, so there is no D-Bus
interface to prefer. The reason is in the module header — keep it, it is what a
reviewer looks for. Its `try`/`catch` around `Gio.Subprocess.new` guards a real
`GError` and is not the unnecessary wrapper the rules below forbid.

Every `link-ctl` call takes the caller's `Gio.Cancellable`, and the toggle
cancels it in `destroy()`. A camera on the end of a USB cable can still be
answering when the panel goes away.
